<?php namespace Phlex\Tool;
	...
	abstract class StringTool {
		...
		public static function getEllipsisedByMaxLength($string, $maxLength, $mode = StringTool::MAX_LENGTH_AFTER, $elipsis = '...') {
			switch ($mode) {
				case StringTool::MAX_LENGTH_AFTER:
					...
					return $result;
					break;
				case StringTool::MAX_LENGTH_BEFORE:
					if (!preg_match_all('/(\S+)?(\s+)?/', $string, $matches)) return $string;
					$chunks = $matches[0];
					$result = implode('', $chunks);
					while($chunks && mb_strlen($result) > $maxLength) {
					...
					return $result;
					break;
				case StringTool::MAX_LENGTH_MIDDLE:
